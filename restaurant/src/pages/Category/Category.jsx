...
const Category = () => {
    ...
    const handleEditCategory = (e) => {
        e.preventDefault();
        updateCategories(
        categories.map((cat) => (cat.id === currentCategory.id ? currentCategory : cat))
        );
    ...
    return (
        <div className="main-content">
                ...
                {filteredCategories.length > 0 ? (
                    ...
                    <tr key={cat.id}>
                        ...
                        <td>
                            ...
                            >
                                {foods.filter((f) => f.category === cat.name).length} items
                            </button>
                        </td>
                        <td>
                        <span className={`status ${cat.status.toLowerCase()}`}>{cat.status}</span>
                        </td>
                        <td>
                        <button className="edit-btn" onClick={() => handleOpenEditModal(cat)}>
                            ✏️
                        </button>
            ...
            {showFoodsModal && (
                <div className="modal-overlay" onClick={() => setShowFoodsModal(false)}>
                    <div className="modal-content" onClick={(e) => e.stopPropagation()}>
                    ...
                    <button className="cancel-btn" onClick={() => setShowFoodsModal(false)}>
                        Close
                    </button>
                    </div>
                </div>
