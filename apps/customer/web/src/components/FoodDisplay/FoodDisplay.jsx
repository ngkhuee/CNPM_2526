...
const FoodDisplay  = ({ filterBy = "category", filterValue = "All", showFilter = false }) => {
  ...
  const filters = ["All", ...Array.from(new Set(food_list.map(f => f[filterBy])))];
  ...
  // Lọc món ăn dựa theo selectedFilter
    ...
    .filter(item => selectedFilter === "All" || item[filterBy] === selectedFilter)
      ...
      <div className='food-display-list'>
        ...
        {/* {filteredRestaurants.map((r, i) => (
          <RestaurantItem 
            key={i} 
            name={r} 
            image={menu_list.find(m => m.menu_name === r)?.menu_image || food_list.find(f => f.restaurant === r)?.image} 
          />
        ))} */}
      </div>
