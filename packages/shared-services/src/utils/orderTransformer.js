/**
 * orderTransformer.js
 * SINGLE SOURCE OF TRUTH for order data transformation
 * Dùng cho TẤT CẢ platforms: customer web, customer mobile, restaurant, admin
 * 
 * Mục tiêu:
 * - Đồng bộ data format giữa API (snake_case) và Frontend (camelCase)
 * - Đảm bảo tất cả fields có đủ cả 2 format để tương thích
 * - Normalize GPS coordinates format
 * - Map foodId/menu_id nhất quán
 */

/**
 * Helper: Normalize GPS format
 * @param {Object|null} gps - GPS object with lat/lng or latitude/longitude
 * @returns {Object|null} Normalized GPS with both formats
 */
 ...
 * Includes BOTH camelCase and snake_case for backward compatibility
 * 
 * @param {Object} apiOrder - Order object from API (snake_case)
 * @param {Object} options - Optional enrichment data
 * @returns {Object} Transformed order with both formats
 */
export const transformOrderFromAPI = (apiOrder, options = {}) => {
    if (!apiOrder) return null;

    const order = {
        // IDs - ALWAYS string for consistency
        ...
        // Items - ALWAYS có foodId mapped từ menu_id
        ...
        // Delivery address - multiple sources
        ...
        // GPS coordinates - normalized format (both variants)
...
/**
 * Transform order from Frontend format to API format (snake_case)
 * 
 * @param {Object} frontendOrder - Order object from frontend (camelCase or mixed)
 * @returns {Object} API-ready order object (snake_case)
 */
 ...
 * @returns {Array<string>} Array of food IDs
 */
