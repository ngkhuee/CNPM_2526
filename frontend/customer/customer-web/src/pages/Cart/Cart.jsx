...
const Cart = () => {
  const { cartItems, food_list, removeFromCart, getTotalCartAmount, url, user, setCartItems } = useContext(StoreContext)
            ...
            return (
              <div key={index}>
                <div className="cart-items-title cart-items-item">
                  ...
                  <p>{(item.price * 1000 * cartItems[item._id]).toLocaleString("vi-VN")}đ</p>
          ...
          <div className="promo-list">
            {promoCodes.map((promo, i) => (
              <div key={i} className="promo-item">
                <span>{promo.code} - Giảm {promo.discount}%</span>
                {appliedPromo && appliedPromo.code === promo.code ? (
                  <button className="remove-btn" onClick={() => setAppliedPromo(null)}>Remove</button>
                ) : (
                  <button className="apply-btn" onClick={() => setAppliedPromo(promo)}>Apply</button>
                )}
              </div>
            ))}
          </div>
