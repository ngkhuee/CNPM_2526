...
export const usePaymentManagement = () => {
    ...
    const fetchWithdrawals = useCallback(async () => {
        ...
        try {
            ...
            const enriched = withdrawalsData.map((w) => {
                const restaurant = restaurantsData.find((r) => r.id === w.restaurant_id);
                return {
                    ...w,
    ...
    const getCompletedWithdrawals = useCallback(() => {
        return withdrawals.filter((w) => w.status === "approved" || w.status === "rejected");
    }, [withdrawals]);
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    if (withdrawal) {
                        ...
                        const balances = await balanceResponse.ok ? await balanceResponse.json() : [];
                        ...
                        if (currentBalance) {
                            ...
                            const newTotalWithdrawn = currentBalance.total_withdrawn + withdrawal.amount;
