...
export const authService = {
  ...
  // Update user profile (for web & mobile)
  async updateProfile(userId, profileData) {
    try {
      const response = await apiClient.put(`/users/${userId}`, profileData);

      // Save updated user to storage with proper mapping
  ...
  // Refresh user data from backend (for sync across devices)
  async refreshProfile(userId) {
    try {
      ...
      if (response) {
        ...
        // Save to storage and update lastSync
        await storage.setItem("user", JSON.stringify(user));
        await storage.setItem("lastProfileSync", Date.now().toString());
